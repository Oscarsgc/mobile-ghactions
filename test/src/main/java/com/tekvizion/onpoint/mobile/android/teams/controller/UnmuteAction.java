                ...
                if (device.elementByIdExists(AndroidConstants.TEAMS_MUTE_BUTTON_ID)) {
                    ...
                    try {
                        new WebDriverWait(device.getDriver(), TapControllerAppInitializer.WEBEX_VIEW_WAITING)
                                .until(ExpectedConditions.presenceOfElementLocated(MobileBy.AccessibilityId(AndroidConstants.TEAMS_UNMUTED_ACCESSIBILITY_ID)));
                        LOGGER.info("Teams Session: " + device.getDriver().getSessionId()
                        ...
                        LOGGER.error("Teams Session: " + device.getDriver().getSessionId()
                                + " | Error on Unmute Action: Button pressed but call was not muted");
                        return "Mute Action Failed.";
                    }
                } else {
                    LOGGER.error("Teams Session: " + device.getDriver().getSessionId()
                            + " | Error on Unmute Action: Unmute button wasn't found");
                    return "Mute Action Failed.";
                }
            } else {
                LOGGER.error("Teams Session: " + device.getDriver().getSessionId()
                        + " | Error on Unmute Action: Calling options bar not found");
                return "Mute Action Failed.";
            }
